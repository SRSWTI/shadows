...
def redis_server(testrun_uid: str, worker_id: str) -> Generator[Container, None, None]:
    ...
    lock_file_name = f"/tmp/shadows-unit-tests-{testrun_uid}-startup"
    ...
    with open(lock_file_name, "w+") as lock_file:
        ...
        containers: Iterable[Container] = cast(
            ...
            client.containers.list(  # type: ignore
                all=True,
                filters={"label": "source=shadows-unit-tests"},
            ),
        )
        for c in containers:
        ...
        with _adminitrative_redis(redis_port) as r:
            r.sadd(f"shadows-unit-tests:{testrun_uid}", worker_id)
    ...
    try:
        ...
        with _adminitrative_redis(redis_port) as r:
            with r.pipeline() as pipe:  # type: ignore
                pipe.srem(f"shadows-unit-tests:{testrun_uid}", worker_id)
                pipe.scard(f"shadows-unit-tests:{testrun_uid}")
                count: int
...
@pytest.fixture
async def shadows(redis_url: str) -> AsyncGenerator[Shadow, None]:
    async with Shadow(name=f"test-shadows-{uuid4()}", url=redis_url) as shadows:
        yield shadows


@pytest.fixture
async def worker(shadows: Shadow) -> AsyncGenerator[Worker, None]:
    async with Worker(
        shadows,
