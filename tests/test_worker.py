...
from shadows.dependencies import Timeout
from shadows.execution import Execution
...
async def test_worker_acknowledges_messages(
    ...
    async with shadows.redis() as redis:
        pending_info = await redis.xpending(
            name=shadows.stream_key,
            groupname=shadows.worker_group_name,
        )
        assert pending_info["pending"] == 0

        assert await redis.xlen(shadows.stream_key) == 0


async def test_two_workers_split_work(shadows: Shadow):
    """Two workers should split the workload"""

    worker1 = Worker(shadows)
    worker2 = Worker(shadows)
...
async def test_worker_reconnects_when_connection_is_lost(
    shadows: Shadow, the_task: AsyncMock
):
    """The worker should reconnect when the connection is lost"""
    worker = Worker(shadows, reconnection_delay=timedelta(milliseconds=100))
...
async def test_worker_respects_concurrency_limit(shadows: Shadow, worker: Worker):
    ...
    for i in range(50):
        await shadows.add(concurrency_tracking_task)(index=i)
...
async def test_worker_handles_redeliveries_from_abandoned_workers(
    ...
    async with Worker(
        shadows, redelivery_timeout=timedelta(milliseconds=100)
    ) as worker_a:
...
async def test_redeliveries_respect_concurrency_limits(shadows: Shadow):
    ...
    await shadows.add(task_that_sometimes_fails)(
    ...
    async with Worker(
        shadows, concurrency=5, redelivery_timeout=timedelta(milliseconds=50)
    ) as worker:
        await worker.run_until_finished()
...
async def test_worker_announcements(
    ...
    async with Worker(shadows, name="worker-a") as worker_a:
        await asyncio.sleep(heartbeat.total_seconds() * 5)

        workers = await shadows.workers()
        assert len(workers) == 1
        assert worker_a.name in {w.name for w in workers}

        async with Worker(shadows, name="worker-b") as worker_b:
            await asyncio.sleep(heartbeat.total_seconds() * 5)
...
async def test_task_announcements(
    ...
    async with Worker(shadows, name="worker-a") as worker_a:
        ...
        workers = await shadows.task_workers("the_task")
        assert len(workers) == 1
        assert worker_a.name in {w.name for w in workers}
...
async def test_worker_recovers_from_redis_errors(
    ...
    monkeypatch.setattr(shadows, "redis", mock_redis)
...
async def test_perpetual_tasks_are_scheduled_close_to_target_time(
    ...
    await shadows.add(perpetual_task, key="my-key")(a="a", b=2)
...
async def test_worker_can_exit_from_perpetual_tasks_that_queue_further_tasks(
    ...
    async def perpetual_task(
        ...
        await shadows.add(inner_task)()
        await shadows.add(inner_task)()

    execution = await shadows.add(perpetual_task)()
...
async def test_worker_can_be_told_to_skip_automatic_tasks(shadows: Shadow):
    ...
    shadows.register(perpetual_task)

    # Without the flag, this would hang because the task would always be scheduled
    async with Worker(shadows, schedule_automatic_tasks=False) as worker:
        await worker.run_until_finished()

    assert not called


async def test_worker_concurrency_limits_task_queuing_behavior(shadows: Shadow):
    ...
    await shadows.add(task_with_concurrency)(customer_id=1)
    await shadows.add(task_with_concurrency)(customer_id=2)
    await shadows.add(task_with_concurrency)(customer_id=1)
    await shadows.add(task_with_concurrency)(customer_id=2)
    await shadows.add(task_with_concurrency)(customer_id=1)

    # Run tasks with reasonable concurrency
    async with Worker(shadows, concurrency=5) as worker:
        await worker.run_until_finished()
...
async def test_worker_concurrency_missing_argument_bypass(shadows: Shadow):
    ...
    await shadows.add(task_missing_concurrency_arg)(customer_id=1)
...
async def test_worker_concurrency_no_limit_early_return(shadows: Shadow):
    ...
    await shadows.add(task_without_concurrency)(customer_id=1)
...
async def test_worker_concurrency_different_customer_branches(shadows: Shadow):
    ...
    for customer_id in [1, 2, 3]:
        await shadows.add(track_customer_execution)(customer_id=customer_id)
...
async def test_worker_concurrency_cleanup_on_success(shadows: Shadow):
    ...
    await shadows.add(successful_task)(customer_id=1)
    await shadows.add(successful_task)(customer_id=1)
    await shadows.add(successful_task)(customer_id=1)
...
async def test_worker_concurrency_cleanup_on_failure(shadows: Shadow):
    ...
    await shadows.add(task_that_may_fail)(customer_id=1, should_fail=True)
    await shadows.add(task_that_may_fail)(customer_id=1, should_fail=False)
    await shadows.add(task_that_may_fail)(customer_id=1, should_fail=False)
...
async def test_worker_concurrency_limits_different_scopes(shadows: Shadow):
    ...
    await shadows.add(scoped_task)(customer_id=1, scope_name="custom")
    await shadows.add(default_scoped_task)(customer_id=1)
...
async def test_worker_concurrency_refresh_mechanism_integration(shadows: Shadow):
    ...
    await shadows.add(long_running_task)(customer_id=1)
    await shadows.add(quick_task)(customer_id=1)

    # Test concurrency mechanism
    worker = Worker(shadows)
...
async def test_worker_concurrency_with_task_failures(shadows: Shadow):
    ...
    # Schedule failing and successful tasks
    await shadows.add(failing_task)(customer_id=1, should_fail=True)
    await shadows.add(failing_task)(customer_id=1, should_fail=False)
    await shadows.add(failing_task)(customer_id=1, should_fail=False)
...
async def test_worker_concurrency_multiple_workers_coordination(shadows: Shadow):
    ...
    for _ in range(4):
        await shadows.add(coordinated_task)(customer_id=1, worker_name="worker1")
    for _ in range(4):
        await shadows.add(coordinated_task)(customer_id=1, worker_name="worker2")

    # Run with two workers
    worker1 = Worker(shadows, name="worker1", concurrency=5)
    worker2 = Worker(shadows, name="worker2", concurrency=5)
...
async def test_worker_concurrency_refresh_handles_redis_errors(shadows: Shadow):
    ...
    with patch.object(shadows, "redis", flaky_redis):
        async with worker:
            await worker.run_until_finished()
...
async def test_worker_concurrency_with_quick_tasks(shadows: Shadow):
...
async def test_worker_handles_concurrent_task_cleanup_gracefully(shadows: Shadow):
    ...
    for _ in range(2):
        await shadows.add(cleanup_test_task)(customer_id=1, should_fail=False)

    # Add one task that will fail to trigger exception handling
    await shadows.add(cleanup_test_task)(customer_id=1, should_fail=True)
...
async def test_worker_concurrency_with_dependencies_integration(shadows: Shadow):
    ...
    await shadows.add(task_with_dependencies)(customer_id=1)

    async with Worker(shadows, name="test-worker") as worker:
        await worker.run_until_finished()
...
async def test_worker_concurrency_robustness_under_stress(shadows: Shadow):
    ...
    for _ in range(20):
        await shadows.add(stress_task)(customer_id=1)

    # Process with multiple workers
    worker1 = Worker(shadows, name="worker1", concurrency=10)
    worker2 = Worker(shadows, name="worker2", concurrency=10)
...
async def test_worker_graceful_shutdown_with_concurrency_management(shadows: Shadow):
...
async def test_worker_concurrency_error_handling_during_execution(shadows: Shadow):
    ...
    await shadows.add(task_that_may_error)(customer_id=1, should_error=True)
    await shadows.add(task_that_may_error)(customer_id=1, should_error=False)
...
async def test_worker_concurrency_cleanup_after_task_completion(shadows: Shadow):
    ...
    await shadows.add(task_with_cleanup_verification)(customer_id=1)
    await shadows.add(task_with_cleanup_verification)(customer_id=1)
    ...
    async with Worker(shadows) as worker:
        ...
        async with shadows.redis() as redis:
            ...
            await redis.keys(f"{shadows.name}:concurrency:*")  # type: ignore
...
async def test_worker_concurrency_edge_cases(shadows: Shadow):
    ...
    for _ in range(5):
        await shadows.add(edge_case_task)(customer_id=1)
...
async def test_worker_timeout_exceeds_redelivery_timeout(shadows: Shadow):
    ...
    async with Worker(shadows, redelivery_timeout=timedelta(milliseconds=100)) as worker:
        await worker.run_until_finished()

    assert task_executed


async def test_worker_concurrency_cleanup_without_dependencies(shadows: Shadow):
...
async def test_worker_concurrency_no_limit_with_custom_shadows(shadows: Shadow):
    """Test early return when task has no concurrency limit using custom shadows."""
    task_executed = False
    ...
    await shadows.add(task_without_concurrency)()
...
async def test_worker_concurrency_missing_argument_early_return(shadows: Shadow):
    ...
    await shadows.add(task_missing_concurrency_arg)()
...
async def test_worker_no_concurrency_dependency_in_function(shadows: Shadow):
...
async def test_worker_no_concurrency_dependency_in_release(shadows: Shadow):
...
async def test_worker_missing_concurrency_argument_in_release(shadows: Shadow):
...
async def test_worker_concurrency_missing_argument_in_can_start(shadows: Shadow):
    ...
    # Register the task
    shadows.register(task_with_missing_concurrency_arg)
...
async def test_worker_exception_before_dependencies(shadows: Shadow):
    ...
    await shadows.add(task_that_will_fail)()

    async with Worker(shadows) as worker:
        # Patch resolved_dependencies to raise an exception immediately
        with patch("shadows.worker.resolved_dependencies") as mock_deps:
...
async def test_finally_block_releases_concurrency_on_success(shadows: Shadow):
...
async def test_replacement_race_condition_stream_tasks(
    ...
    await shadows.add(the_task, now(), key=key)("a", "b", c="c")
...
async def test_rapid_replace_operations(
    ...
    for i in range(5):
        ...
        await shadows.replace(the_task, when, key=key)(f"arg{i}", b=f"b{i}")
...
async def test_wrongtype_error_with_legacy_known_task_key(
    ...
    HSET on known task keys that existed as string values from older shadows versions.

    The original error occurred when:
    1. A legacy shadows created known task keys as simple string values (timestamps)
    ...
    # This is what older versions of shadows would have done
    async with shadows.redis() as redis:
        known_task_key = shadows.known_task_key(key)
        ...
        await redis.zadd(shadows.queue_key, {key: when.timestamp()})

        await redis.hset(  # type: ignore
            shadows.parked_task_key(key),
            mapping={
                "key": key,
                "when": when.isoformat(),
                "function": "trace",
...
class KeyCountChecker:
    ...
    def __init__(self, shadows: Shadow, redis: Redis) -> None:
    ...
    async def verify_keys_increased(self, operation: str) -> None:
        ...
        current_counts = await count_redis_keys_by_type(self.redis, self.shadows.name)
    ...
    async def verify_keys_returned_to_baseline(self, operation: str) -> None:
        ...
        final_counts = await count_redis_keys_by_type(self.redis, self.shadows.name)
...
async def test_redis_key_cleanup_successful_task(
    ...
    shadows.register(successful_task)

    async with shadows.redis() as redis:
        checker = KeyCountChecker(shadows, redis)
        await checker.capture_baseline()

        # Schedule the task
        await shadows.add(successful_task)()
...
async def test_redis_key_cleanup_failed_task(shadows: Shadow, worker: Worker) -> None:
    ...
    async with shadows.redis() as redis:
        ...
        # Schedule the task
        await shadows.add(failing_task)()
        await checker.verify_keys_increased("scheduling")
...
async def test_redis_key_cleanup_cancelled_task(shadows: Shadow, worker: Worker) -> None:
    ...
    shadows.register(task_to_cancel)
    ...
    async with shadows.redis() as redis:
        ...
        execution = await shadows.add(task_to_cancel, future_time)()
        await checker.verify_keys_increased("scheduling")
...
async def test_replace_task_with_legacy_known_key(
    ...
    await shadows.replace("trace", replacement_time, key=key)("replacement message")
