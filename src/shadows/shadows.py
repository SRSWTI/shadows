...
class Shadow:
    """A Shadow represents a collection of tasks that may be scheduled for later
    execution.  With a Shadow, you can add, replace, and cancel tasks.
    Example:
    ...
    async with Shadow() as shadows:
        shadows.add(my_task)("Hello", recipient="world")
    ```
    """
    ...
    def register(self, function: TaskFunction) -> None:
        """Register a task with the Shadow.
    ...
    def replace(
        ...
        """Replace a previously scheduled task on the Shadow.
    ...
    async def cancel(self, key: str) -> None:
        """Cancel a previously scheduled task on the Shadow.
        ...
        with tracer.start_as_current_span(
            "shadows.cancel",
            attributes={**self.labels(), "shadows.key": key},
        ):
            async with self.redis() as redis:
    ...
    async def restore(
        ...
        """Restore a previously stricken task to the Shadow.
    ...
    async def _send_strike_instruction(self, instruction: StrikeInstruction) -> None:
        with tracer.start_as_current_span(
            f"shadows.{instruction.direction}",
            attributes={
                **self.labels(),
                **instruction.labels(),
            },
    ...
    async def _monitor_strikes(self) -> NoReturn:
        ...
        while True:
            try:
                ...
                REDIS_DISRUPTIONS.add(1, {"shadows": self.name})
    ...
    async def snapshot(self) -> ShadowSnapshot:
        """Get a snapshot of the Shadow, including which tasks are scheduled or currently
        ...
        return ShadowSnapshot(now, total_tasks, future, running, workers)
    ...
    async def workers(self) -> Collection[WorkerInfo]:
        """Get a list of all workers that have sent heartbeats to the Shadow.

        Returns:
            A list of all workers that have sent heartbeats to the Shadow.
        """
    ...
    async def clear(self) -> int:
        """Clear all pending and scheduled tasks from the shadows.
