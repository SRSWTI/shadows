...
class Worker:
    """A Worker executes tasks on a Shadow.  You may run as many workers as you like
    to work a single Shadow.
    ...
    def labels(self) -> Mapping[str, str]:
        return {
            **self.shadows.labels(),
            "shadows.worker": self.name,
        }

    def _log_context(self) -> Mapping[str, str]:
        return {
            **self.labels(),
            "shadows.queue_key": self.shadows.queue_key,
            "shadows.stream_key": self.shadows.stream_key,
        }

    @classmethod
    async def run(
        cls,
        shadows_name: str = "shadows",
    ...
    async def run_at_most(self, iterations_by_key: Mapping[str, int]) -> None:
        ...
        self.shadows.strike_list.add_condition(has_reached_max_iterations)
        try:
            await self.run_until_finished()
        finally:
            self.shadows.strike_list.remove_condition(has_reached_max_iterations)
            self._execution_counts = {}
    ...
    async def _run(self, forever: bool = False) -> None:
        ...
        while True:
            try:
                async with self.shadows.redis() as redis:
    ...
    async def _worker_loop(self, redis: Redis, forever: bool = False):
        ...
        async def check_for_work() -> bool:
            ...
            async with redis.pipeline() as pipeline:
                pipeline.xlen(self.shadows.stream_key)
                pipeline.zcard(self.shadows.queue_key)
        ...
        async def get_redeliveries(redis: Redis) -> RedisReadGroupResponse:
            ...
            _, redeliveries, *_ = await redis.xautoclaim(
                name=self.shadows.stream_key,
                groupname=self.shadows.worker_group_name,
        ...
        async def get_new_deliveries(redis: Redis) -> RedisReadGroupResponse:
            ...
            return await redis.xreadgroup(
                groupname=self.shadows.worker_group_name,
                consumername=self.name,
                streams={self.shadows.stream_key: ">"},
        ...
        def start_task(
            ...
            if not (function := self.shadows.tasks.get(function_name)):
                logger.warning(
        ...
        async def ack_message(redis: Redis, message_id: RedisMessageID) -> None:
            ...
            async with redis.pipeline() as pipeline:
                pipeline.xack(
                    self.shadows.stream_key,
                    self.shadows.worker_group_name,
                    message_id,
                )
                pipeline.xdel(
                    self.shadows.stream_key,
        ...
        while not worker_stopping.is_set() or total_work:
            try:
                ...
                total_work, due_work = await stream_due_tasks(
                    keys=[self.shadows.queue_key, self.shadows.stream_key],
                    args=[datetime.now(timezone.utc).timestamp(), self.shadows.name],
                )
        ...
        async with self.shadows.redis() as redis:
            try:
                async with redis.lock(
                    f"{self.shadows.name}:perpetual:lock", timeout=10, blocking=False
                ):
                    for task_function in self.shadows.tasks.values():
                        ...
                        await self.shadows.add(task_function, key=key)()
    ...
    async def _delete_known_task(
        ...
        known_task_key = self.shadows.known_task_key(key)
        stream_id_key = self.shadows.stream_id_key(key)
    ...
    async def _execute(self, execution: Execution) -> None:
        ...
        if self.shadows.strike_list.is_stricken(execution):
            async with self.shadows.redis() as redis:
                await self._delete_known_task(redis, execution)

            logger.warning("🗙 %s", call, extra=log_context)
            TASKS_STRICKEN.add(1, counter_labels | {"shadows.where": "worker"})
            return
                    ...
                    if concurrency_limit and not concurrency_limit.is_bypassed:
                        async with self.shadows.redis() as redis:
                            ...
                            if not await self._can_start_task(redis, execution):
                                ...
                                await self.shadows.add(execution.function, when=when)(
    ...
    async def _retry_if_requested(
        ...
        execution.attempt += 1
        await self.shadows.schedule(execution)
    ...
    async def _perpetuate_if_requested(
        ...
        if perpetual.cancelled:
            await self.shadows.cancel(execution.key)
            return False

        now = datetime.now(timezone.utc)
        when = max(now, now + perpetual.every - (duration or timedelta(0)))

        await self.shadows.replace(execution.function, when, execution.key)(
            *perpetual.args,
            **perpetual.kwargs,
        )
    ...
    def _startup_log(self) -> None:
        ...
        for task_name, task in self.shadows.tasks.items():
            logger.info("* %s(%s)", task_name, compact_signature(get_signature(task)))

    @property
    def workers_set(self) -> str:
        return self.shadows.workers_set

    def worker_tasks_set(self, worker_name: str) -> str:
        return self.shadows.worker_tasks_set(worker_name)

    def task_workers_set(self, task_name: str) -> str:
        return self.shadows.task_workers_set(task_name)

    async def _heartbeat(self) -> None:
        while True:
            await asyncio.sleep(self.shadows.heartbeat_interval.total_seconds())
            try:
                now = datetime.now(timezone.utc).timestamp()
                maximum_age = (
                    self.shadows.heartbeat_interval * self.shadows.missed_heartbeats
                )
                oldest = now - maximum_age.total_seconds()

                task_names = list(self.shadows.tasks)

                async with self.shadows.redis() as r:
                    ...
                    async with r.pipeline() as pipeline:
                        pipeline.xlen(self.shadows.stream_key)
                        pipeline.zcount(self.shadows.queue_key, 0, now)
                        pipeline.zcount(self.shadows.queue_key, now, "+inf")
                        ...
                        QUEUE_DEPTH.set(
                            stream_depth + overdue_depth, self.shadows.labels()
                        )
                        SCHEDULE_DEPTH.set(schedule_depth, self.shadows.labels())
    ...
    async def _can_start_task(self, redis: Redis, execution: Execution) -> bool:
        ...
        scope = concurrency_limit.scope or self.shadows.name
        concurrency_key = (
