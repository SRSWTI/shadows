...
class Execution:
    ...
    def general_labels(self) -> Mapping[str, str]:
        return {"shadows.task": self.function.__name__}

    def specific_labels(self) -> Mapping[str, str | int]:
        return {
            "shadows.task": self.function.__name__,
            "shadows.key": self.key,
            "shadows.when": self.when.isoformat(),
            "shadows.attempt": self.attempt,
        }
...
class StrikeInstruction(abc.ABC):
    ...
    def labels(self) -> Mapping[str, str]:
        labels: dict[str, str] = {}
        if self.function:
            labels["shadows.task"] = self.function

        if self.parameter:
            labels["shadows.parameter"] = self.parameter
            labels["shadows.operator"] = self.operator
            labels["shadows.value"] = repr(self.value)
