...
from .shadows import Shadow, ShadowSnapshot, WorkerInfo
from .execution import Operator
from .worker import Worker

app: typer.Typer = typer.Typer(
    help="Shadow - A distributed background task system for Python functions",
    add_completion=True,
...
def worker(
    tasks: Annotated[
        ...
        typer.Option(
            "--tasks",
            help=(
                "The dotted path of a task collection to register with the shadows. "
                "This can be specified multiple times.  A task collection is any "
                "iterable of async functions."
            ),
            envvar="SHADOW_TASKS",
        ),
    ] = ["shadows.tasks:standard_tasks"],
    shadows_: Annotated[
        str,
        typer.Option(
            "--shadows",
            help="The name of the shadows",
            envvar="SHADOW_NAME",
        ),
    ] = "shadows",
    url: Annotated[
    ...
    concurrency: Annotated[
        ...
        typer.Option(
            ...
            envvar="SHADOW_WORKER_CONCURRENCY",
        ),
    ] = 10,
    redelivery_timeout: Annotated[
        ...
        typer.Option(
            ...
            envvar="SHADOW_WORKER_REDELIVERY_TIMEOUT",
        ),
    ] = timedelta(minutes=5),
    reconnection_delay: Annotated[
        ...
        typer.Option(
            ...
            envvar="SHADOW_WORKER_RECONNECTION_DELAY",
        ),
    ] = timedelta(seconds=5),
    minimum_check_interval: Annotated[
        ...
        typer.Option(
            ...
            envvar="SHADOW_WORKER_MINIMUM_CHECK_INTERVAL",
        ),
    ...
    scheduling_resolution: Annotated[
        ...
        typer.Option(
            ...
            envvar="SHADOW_WORKER_SCHEDULING_RESOLUTION",
        ),
    ...
    until_finished: Annotated[
        ...
        typer.Option(
            "--until-finished",
            help="Exit after the current shadows is finished",
        ),
    ] = False,
    healthcheck_port: Annotated[
        ...
        typer.Option(
            ...
            envvar="SHADOW_WORKER_HEALTHCHECK_PORT",
        ),
    ] = None,
    metrics_port: Annotated[
        ...
        typer.Option(
            ...
            envvar="SHADOW_WORKER_METRICS_PORT",
        ),
    ] = None,
) -> None:
...
@app.command(help="Strikes a task or parameters from the shadows")
def strike(
    function: Annotated[
        str,
        typer.Argument(
    ...
    value: Annotated[
        str | None,
        typer.Argument(
            help="The value to strike from the shadows",
        ),
    ] = None,
    shadows_: Annotated[
        str,
    ...
    async def run() -> None:
        async with Shadow(name=shadows_, url=url) as shadows:
            await shadows.strike(function, parameter, operator, value_)

    asyncio.run(run())


@app.command(help="Clear all pending and scheduled tasks from the shadows")
def clear(
    shadows_: Annotated[
        str,
        typer.Option(
    ...
    async def run() -> None:
        async with Shadow(name=shadows_, url=url) as shadows:
            cleared_count = await shadows.clear()
            print(f"Cleared {cleared_count} tasks from shadows '{shadows_}'")

    asyncio.run(run())


@app.command(help="Restores a task or parameters to the Shadow")
def restore(
    function: Annotated[
        str,
        typer.Argument(
    ...
    value: Annotated[
        str | None,
        typer.Argument(
            help="The value to restore to the shadows",
        ),
    ] = None,
    shadows_: Annotated[
        str,
    ...
    async def run() -> None:
        async with Shadow(name=shadows_, url=url) as shadows:
            await shadows.restore(function, parameter, operator, value_)

    asyncio.run(run())


tasks_app: typer.Typer = typer.Typer(
    help="Run shadows's built-in tasks", no_args_is_help=True
)
app.add_typer(tasks_app, name="tasks")


@tasks_app.command(help="Adds a trace task to the Shadow")
def trace(
    shadows_: Annotated[
        str,
        typer.Option(
    ...
    delay: Annotated[
        ...
        typer.Option(
            parser=duration,
            help="The delay before the task is added to the shadows",
        ),
    ] = timedelta(seconds=0),
) -> None:
    async def run() -> None:
        async with Shadow(name=shadows_, url=url) as shadows:
            ...
            execution = await shadows.add(tasks.trace, when)(message)
            print(
                f"Added {execution.function.__name__} task {execution.key!r} to "
                f"the shadows {shadows.name!r}"
            )

    asyncio.run(run())


@tasks_app.command(help="Adds a fail task to the Shadow")
def fail(
    shadows_: Annotated[
        str,
        typer.Option(
    ...
    async def run() -> None:
        async with Shadow(name=shadows_, url=url) as shadows:
            when = datetime.now(timezone.utc) + delay
            execution = await shadows.add(tasks.fail, when)(message)
            print(
                f"Added {execution.function.__name__} task {execution.key!r} to "
                f"the shadows {shadows.name!r}"
            )
...
@tasks_app.command(help="Adds a sleep task to the Shadow")
def sleep(
    shadows_: Annotated[
        str,
        typer.Option(
    ...
    async def run() -> None:
        async with Shadow(name=shadows_, url=url) as shadows:
            when = datetime.now(timezone.utc) + delay
            execution = await shadows.add(tasks.sleep, when)(seconds)
            print(
                f"Added {execution.function.__name__} task {execution.key!r} to "
                f"the shadows {shadows.name!r}"
            )
...
@app.command(help="Shows a snapshot of what's on the shadows right now")
def snapshot(
    tasks: Annotated[
        list[str],
        typer.Option(
    ...
    async def run() -> ShadowSnapshot:
        async with Shadow(name=shadows_, url=url) as shadows:
            for task_path in tasks:
                shadows.register_collection(task_path)

            return await shadows.snapshot()
...
workers_app: typer.Typer = typer.Typer(
    help="Look at the workers on a shadows", no_args_is_help=True
)
...
def print_workers(
    ...
    table = Table(title=f"Workers in Shadow: {shadows_name}")
...
@workers_app.command(name="ls", help="List all workers on the shadows")
def list_workers(
    shadows_: Annotated[
        str,
        typer.Option(
    ...
    async def run() -> Collection[WorkerInfo]:
        async with Shadow(name=shadows_, url=url) as shadows:
            return await shadows.workers()

    workers = asyncio.run(run())

    print_workers(shadows_, workers)


@workers_app.command(
    name="for-task",
    help="List the workers on the shadows that can process a certain task",
)
def workers_for_task(
    ...
    async def run() -> Collection[WorkerInfo]:
        async with Shadow(name=shadows_, url=url) as shadows:
            return await shadows.task_workers(task)

    workers = asyncio.run(run())

    print_workers(shadows_, workers, highlight_task=task)
